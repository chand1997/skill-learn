...
const SectionList = ({ items, onReorder, onEdit }: SectionListProps) => {
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    setSections(items);
    const bulkUpdateData = updatedItems.map((section) => ({
      id: section.id,
              ...
              >
                {(provided) => (
                  <div
                    ...
                    ref={provided.innerRef}
                    className="flex items-center bg-green-100/90 text-sm font-medium p-3 rounded-lg"
                  >
                    <div {...provided.dragHandleProps}>
                      <Grip className="h-4 w-4 cursor-pointer mr-4 hover:text-pink-300" />
                    </div>
                    {section.title}
                    <div className="ml-auto">
                      <Pencil
                        className="h-4 w-4 cursor-pointer hover:text-pink-300"
